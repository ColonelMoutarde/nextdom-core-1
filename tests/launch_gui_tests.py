...
def migration_tests():
    ...
    print('B')
    exec_command_in_container(container_name, 'php /var/www/html/install/migrate_jeedom_to_nextdom.php backup=/var/www/html/backup/backup-Jeedom-3.2.11-2018-11-17-23h26.tar.gz > /dev/null 2>&1') #pylint: disable=line-too-long
    # Reset admin password
    print('C')
    exec_command_in_container(container_name, '/usr/bin/mysql -u root nextdomdev <<< "UPDATE user SET password = SHA2(\'nextdom-test\', 512)"') #pylint: disable=line-too-long
